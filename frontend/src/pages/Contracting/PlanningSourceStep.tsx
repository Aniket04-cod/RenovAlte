...
const PlanningSourceStep: React.FC<PlanningSourceStepProps> = ({
	...
	const handleNext = async () => {
		...
		try {
			...
			setError(err instanceof Error ? err.message : "Failed to submit requirements. Please try again.");
			setIsSubmitting(false);
		}
	};
	...
	const handleSubmitAnswers = async () => {
		...
		try {
			...
			await contractingPlanningApi.submitAnswers(selectedProject.id, userAnswers);
	...
	if (isLoading) {
		return (
			<div className="flex items-center justify-center py-12">
				...
				<Text className="ml-3 text-gray-600">Loading planning requirements...</Text>
			</div>
		);
	}
		...
		<div className="space-y-3">
				...
				<div className="bg-white border-2 border-emerald-200 rounded-lg p-6 sm:p-8 space-y-6">
					...
					<div>
						<label htmlFor="description" className="block mb-2">
							...
							<Text className="text-gray-600 text-sm">
								Describe your specific contracting needs, requirements, or challenges
							</Text>
						</label>
						<textarea
							id="description"
					...
					<div>
						...
						<Text className="text-gray-600 text-sm mb-3">
							Share any relevant photos, plans, or documents to help contractors understand your needs
						</Text>
						...
						<div className="relative">
							...
							>
								...
								<span>{existingPlanning ? 'Upload More Files' : 'Choose Files'}</span>
							</label>
						</div>
						...
						>
							{isSubmitting ? (
								<>
									...
									<span>{existingPlanning ? 'Updating...' : 'Submitting...'}</span>
								</>
							) : (
								<>
									<span>{existingPlanning ? 'Update & Continue' : 'Continue'}</span>
			...
			>
				<div className="space-y-6">
					...
					{/* Questions Section */}
					{existingPlanning?.ai_questions && existingPlanning.ai_questions.length > 0 && (
						<div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
							{/* Header */}
							<div className="bg-gradient-to-r from-emerald-500 to-teal-500 px-5 sm:px-6 py-5 sm:py-6">
								<Heading level={2} className="text-xl sm:text-2xl text-white mb-1.5">
									A few quick questions
								</Heading>
								<Text className="text-emerald-50 text-xs sm:text-sm">
									Help us match you with the perfect contractors for your project
								</Text>
							</div>
							...
							<div className="p-5 sm:p-6 space-y-5">
								{existingPlanning.ai_questions.map((question, index) => (
									...
									>
												...
												question.options.map((option) => {
													...
													return (
														...
														>
															...
															{/* Option Text */}
															<Text className={`flex-1 text-xs sm:text-sm transition-colors ${
																isSelected
										...
										{existingPlanning?.ai_questions && index < existingPlanning.ai_questions.length - 1 && (
											<div className="pt-2">
							...
							{error && (
								<div className="mx-5 sm:mx-6 mb-5 bg-red-50 border-l-4 border-red-500 rounded-r-lg p-3">
									<Text className="text-red-800 text-xs sm:text-sm font-medium">{error}</Text>
								</div>
							)}
